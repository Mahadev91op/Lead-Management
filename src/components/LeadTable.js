...
export default function LeadTable({ leads, onEdit, onDelete, onSort, sortConfig, onUpdateStatus }) {
  ...
  return (
    <div className="overflow-x-auto bg-slate-900/60 border border-slate-800 rounded-2xl backdrop-blur-xl shadow-xl">
      <table className="w-full text-left border-collapse min-w-[900px]">
        <thead>
          <tr className="border-b border-slate-800 text-slate-400 text-xs uppercase tracking-wider bg-slate-950/50">
            <th onClick={() => handleHeaderClick('name')} className="p-4 font-medium cursor-pointer hover:text-white transition"><div className="flex items-center">Client <SortIcon column="name"/></div></th>
            <th onClick={() => handleHeaderClick('priority')} className="p-4 font-medium cursor-pointer hover:text-white transition"><div className="flex items-center">Priority <SortIcon column="priority"/></div></th>
            <th onClick={() => handleHeaderClick('budget')} className="p-4 font-medium cursor-pointer hover:text-white transition"><div className="flex items-center">Budget <SortIcon column="budget"/></div></th>
            <th onClick={() => handleHeaderClick('status')} className="p-4 font-medium cursor-pointer hover:text-white transition"><div className="flex items-center">Status <SortIcon column="status"/></div></th>
            <th onClick={() => handleHeaderClick('createdAt')} className="p-4 font-medium cursor-pointer hover:text-white transition"><div className="flex items-center">Date <SortIcon column="createdAt"/></div></th>
            ...
            >
              ...
              <td className="p-4 text-right">
                <div className="flex justify-end gap-2">
                    ...
                    <button onClick={() => onDelete(lead._id)} className="text-slate-400 hover:text-red-400 p-2 bg-slate-800/50 hover:bg-red-500/10 rounded transition border border-transparent hover:border-red-500/20" title="Delete">
                        <Trash2 size={16} />
                    </button>
                </div>
              </td>
